...
(deftest shuffle-test
  ...
  (is (=
       ...
       (lc/apply-linear-function lc/tensor-m1324 (lc/otimes (coproduct (->ShuffleWord [11 77])) (coproduct (->ShuffleWord [22]))))))
  ;- condition on antipode: \Nabla (A \otimes id) \Delta = \eta \vareps
  (is (= {(->ShuffleWord []) 1}
         (lc/apply-linear-function lc/tensor-m12 (lc/apply-linear-function lc/tensor-antipode-otimes-id (coproduct (->ShuffleWord []))))))
  (is (= {}
         (lc/apply-linear-function lc/tensor-m12 (lc/apply-linear-function lc/tensor-antipode-otimes-id (coproduct (->ShuffleWord [1]))))))
  (is (= {}
         (lc/apply-linear-function lc/tensor-m12 (lc/apply-linear-function lc/tensor-antipode-otimes-id (coproduct (->ShuffleWord [1 2]))))))

  (let [tau {(->ShuffleWord []) 17, (->ShuffleWord [3]) -5, (->ShuffleWord [1 3 2]) 22}]
    ;- unit
    (is (=
         (lc/* 17 tau)
         (lc/* (unit->ShuffleWord 17) tau)))
    ;- counit
    (is (=
         (lc/apply-linear-function
           lc/tensor-id-otimes-counit
           (lc/coproduct tau))
         tau)))
  )
...
(deftest concat-test
  ...
  (is (=
       ...
       (lc/apply-linear-function lc/tensor-m1324 (lc/otimes (coproduct (->ConcatWord [1])) (coproduct (->ConcatWord [2 3]))))))
  (is (= {(->ConcatWord []) 1}
         (lc/apply-linear-function lc/tensor-m12 (lc/apply-linear-function lc/tensor-antipode-otimes-id (coproduct (->ConcatWord []))))))
  (is (= {}
         (lc/apply-linear-function lc/tensor-m12 (lc/apply-linear-function lc/tensor-antipode-otimes-id (coproduct (->ConcatWord [4]))))))

  (is (= {}
         (lc/apply-linear-function lc/tensor-m12 (lc/apply-linear-function lc/tensor-antipode-otimes-id (coproduct (->ConcatWord [4 5 6]))))))
